  ...
  private renderContent(data: PopupData): string {
    switch (data.type) {
      ...
      case 'travelDestination':
        return this.renderTravelDestinationPopup(data.data as any);
      default:
        return '';
    }
  }

  private renderTravelDestinationPopup(dest: any): string {
    const variant = dest.sourceVariant || 'full';

    // Default CTA (Travel context)
    let ctaLinks = `
      <a href="${sanitizeUrl(`https://www.google.com/search?q=flights+to+${dest.name}`)}" target="_blank" class="popup-link" style="display:block;margin-bottom:8px;padding:8px;background:var(--bg-elevated);border-radius:6px;text-align:center;font-weight:bold;">✈️ Find Flights to ${escapeHtml(dest.name)}</a>
      <a href="${sanitizeUrl(`https://www.booking.com/searchresults.html?ss=${dest.name}`)}" target="_blank" class="popup-link" style="display:block;padding:8px;background:var(--bg-elevated);border-radius:6px;text-align:center;font-weight:bold;">🏨 Book Hotels in ${escapeHtml(dest.name)}</a>
    `;

    // Dynamic CTA based on Affiliate Variant Context
    if (variant === 'sports') {
      ctaLinks = `
        <a href="${sanitizeUrl(`https://www.ticketmaster.com/search?q=${dest.name}+sports`)}" target="_blank" class="popup-link" style="display:block;margin-bottom:8px;padding:8px;background:var(--bg-elevated);border-radius:6px;text-align:center;font-weight:bold;">🎟️ Buy Match Tickets (${escapeHtml(dest.name)})</a>
        <a href="${sanitizeUrl(`https://www.booking.com/searchresults.html?ss=${dest.name}`)}" target="_blank" class="popup-link" style="display:block;padding:8px;background:var(--bg-elevated);border-radius:6px;text-align:center;font-weight:bold;">🏨 Book Nearby Hotels</a>
      `;
    } else if (variant === 'climate' || variant === 'weather') {
      ctaLinks = `
        <a href="${sanitizeUrl(`https://weather.com/weather/today/l/${dest.lat},${dest.lon}`)}" target="_blank" class="popup-link" style="display:block;margin-bottom:8px;padding:8px;background:var(--bg-elevated);border-radius:6px;text-align:center;font-weight:bold;">☁️ Live Local Forecast</a>
        <a href="${sanitizeUrl(`https://www.google.com/search?q=emergency+preparedness+kits+${dest.name}`)}" target="_blank" class="popup-link" style="display:block;padding:8px;background:var(--bg-elevated);border-radius:6px;text-align:center;font-weight:bold;">🎒 Disaster Prep Gear</a>
      `;
    } else if (variant === 'finance') {
      ctaLinks = `
        <a href="${sanitizeUrl(`https://finance.yahoo.com/lookup?s=${dest.name}`)}" target="_blank" class="popup-link" style="display:block;margin-bottom:8px;padding:8px;background:var(--bg-elevated);border-radius:6px;text-align:center;font-weight:bold;">📈 Local Market Data</a>
        <a href="${sanitizeUrl(`https://www.bloomberg.com/search?query=${dest.name}`)}" target="_blank" class="popup-link" style="display:block;padding:8px;background:var(--bg-elevated);border-radius:6px;text-align:center;font-weight:bold;">📰 Regional Economic News</a>
      `;
    }

    return `
      <div class="popup-header hotspot">
        <span class="popup-title">${escapeHtml(dest.emoji)} ${escapeHtml(dest.name.toUpperCase())}</span>
        <span class="popup-badge medium">${escapeHtml(dest.region)}</span>
        <button class="popup-close">×</button>
      </div>
      <div class="popup-body">
        <p class="popup-description">${escapeHtml(dest.description || 'Global hub of significant activity.')}</p>
        <div class="popup-stats">
          <div class="popup-stat">
            <span class="stat-label">Safety</span>
            <span class="stat-value">${escapeHtml(dest.safetyLevel || 'Normal')}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">Best To Visit</span>
            <span class="stat-value">${escapeHtml(dest.bestMonths || 'All year')}</span>
          </div>
        </div>
        <div class="popup-section why-matters-section">
          <span class="section-label">Top Attractions</span>
          <div class="popup-tags">
            ${(dest.tags || []).map((t: string) => `<span class="popup-tag">${escapeHtml(t)}</span>`).join('')}
          </div>
        </div>
