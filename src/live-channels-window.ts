...
export function initLiveChannelsWindow(containerEl?: HTMLElement): void {
  ...
  function showEditForm(row: HTMLElement, ch: LiveChannel, listEl: HTMLElement): void {
    ...
    saveBtn.addEventListener('click', () => {
      const displayName = nameInput.value.trim() || ch.name || ch.handle || 'Custom';
  ...
  let customTitle = t('components.liveNews.customChannel') ?? 'Custom channel';
  if (SITE_VARIANT === 'travel') customTitle = 'Add custom travel news source (YouTube / X / RSS)';
  else if (SITE_VARIANT === 'climate' || SITE_VARIANT === 'weather') customTitle = 'Add custom weather/climate source';
  else if (SITE_VARIANT === 'health') customTitle = 'Add custom health/outbreak source';
  else if (SITE_VARIANT === 'sports') customTitle = 'Add custom sports/event channel';
    ...
    <div class="live-channels-window-shell">
      ...
      <div class="live-channels-window-content">
        ...
        <div class="live-news-manage-add-section">
          <span class="live-news-manage-add-title">${escapeHtml(customTitle)}</span>
