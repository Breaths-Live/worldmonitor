...
Sentry.init({
  ...
  environment: (location.hostname === 'worldmonitor.app' || location.hostname.endsWith('breaths.live') || location.hostname.endsWith('breaths.me')) ? 'production'
...
// Validate API keys at startup (warns in console, never crashes)
...
if (urlParams.get('settings') === '1') {
  ...
  app
    ...
    .then(() => {
      ...
      // Initialize donate popup (appears every 20 minutes)
      new DonatePopup();
    })
    .catch(console.error);
}
