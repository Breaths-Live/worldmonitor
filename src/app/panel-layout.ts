...
import { TravelSidebar } from '@/features/travel/TravelSidebar';
...
import { renderWorldAffSection } from '@/features/affiliate/sections/WorldAffSection';
import { renderTechAffSection } from '@/features/affiliate/sections/TechAffSection';
import { renderFinanceAffSection } from '@/features/affiliate/sections/FinanceAffSection';
import { renderGoodNewsAffSection } from '@/features/affiliate/sections/GoodNewsAffSection';
import { renderHealthAffSection } from '@/features/affiliate/sections/HealthAffSection';
import { renderSportsAffSection } from '@/features/affiliate/sections/SportsAffSection';
import { renderWeatherAffSection } from '@/features/affiliate/sections/WeatherAffSection';
import { renderTravelAffSection } from '@/features/affiliate/sections/TravelAffSection';
import { createFooter } from '@/components/Footer';
import { safeRenderAffSection } from '@/components/AffErrorBoundary';
import { applyVariantMeta } from '@/app/variantMeta';
import { createTabNav } from '@/features/lucky/components/TabNav';
import { renderExploreTab } from '@/features/lucky/tabs/ExploreTab';
import { renderStayTab } from '@/features/lucky/tabs/StayTab';
import { renderDoTab } from '@/features/lucky/tabs/DoTab';
import { renderMoveTab } from '@/features/lucky/tabs/MoveTab';
...
export class PanelLayoutManager implements AppModule {
  ...
  init(): void {
    this.renderLayout();
    applyVariantMeta(SITE_VARIANT);
    this.setupLuckyTabSwitching();
  }
    ...
    this.ctx.container.innerHTML = `
      <div class="header">
        ...
        return `
          ...
          <a href="https://x.com/victorchuyen" target="_blank" rel="noopener" class="credit-link">
            <svg class="x-logo" width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            <span class="credit-text">@victorchuyen</span>
          </a>
          <a href="https://github.com/tncsharetool/worldmonitor" target="_blank" rel="noopener" class="github-link" title="${t('header.viewOnGitHub')}">
      ...
      <div class="main-content">
        ...
        ${SITE_VARIANT === 'travel' ? `
        <div class="lucky-tabs-container" id="luckyTabsContainer" style="padding: 0 16px; margin-top: 16px;">
          ${createTabNav('explore')}
          <div id="luckyTabContent">
            ${safeRenderAffSection(() => renderExploreTab(), 'Lucky Explore Tab')}
          </div>
        </div>
        ` : ''}
        <div class="affiliate-container" id="affiliateContainer" style="padding: 0 16px;">
          ${safeRenderAffSection(() => {
          switch (SITE_VARIANT) {
            case 'tech': return renderTechAffSection();
            case 'finance': return renderFinanceAffSection();
            case 'travel': return renderTravelAffSection();
            case 'climate': return renderWeatherAffSection();
            case 'health': return renderHealthAffSection();
            case 'sports': return renderSportsAffSection();
            case 'happy': return renderGoodNewsAffSection();
            case 'full':
            default:
              return renderWorldAffSection();
          }
        }, 'Affiliate Section')}
        </div>
      </div>
  ...
  private createPanels(): void {
    ...
    if (SITE_VARIANT === 'health') {
      this.ctx.panels['health-risk-monitor'] = new DummyPanel({ id: 'health-risk-monitor', title: 'Health Risk Monitor' });
      this.ctx.panels['outbreak-alerts'] = new DummyPanel({ id: 'outbreak-alerts', title: 'Outbreak Alerts' });
    }

    // Tech dummy panels
    if (SITE_VARIANT === 'tech') {
      this.ctx.panels['cyber-threat-feed'] = new DummyPanel({ id: 'cyber-threat-feed', title: 'Cyber Threat Feed' });
    }

    // Sports panels
    if (SITE_VARIANT === 'sports') {
      this.ctx.panels['sports-events'] = new DummyPanel({ id: 'sports-events', title: 'Sports & Events' });
      this.ctx.panels['live-scores'] = new DummyPanel({ id: 'live-scores', title: 'Live Scores' });
    }

    // Travel variant panels
    if (SITE_VARIANT === 'travel') {
      const travelSidebar = new TravelSidebar();
      this.ctx.panels['travel-sidebar'] = travelSidebar;
    }
  ...
  private setupLuckyTabSwitching(): void {
    if (SITE_VARIANT !== 'travel') return;
    const container = document.getElementById('luckyTabsContainer');
    if (!container) return;

    const luckyTabRenderers: Record<string, () => string> = {
    ...
    container.addEventListener('click', (e) => {
      const btn = (e.target as HTMLElement).closest<HTMLElement>('[data-lucky-tab]');
      if (!btn) return;
      const tab = btn.dataset.luckyTab;
      if (!tab) return;

      // Update active state on buttons
      container.querySelectorAll('.lucky-tab-btn').forEach(b => {
        const isActive = (b as HTMLElement).dataset.luckyTab === tab;
        (b as HTMLElement).style.background = isActive ? 'var(--accent-alpha, rgba(68,136,255,0.15))' : 'transparent';
        (b as HTMLElement).style.color = isActive ? 'var(--accent-color, #4488ff)' : 'var(--text-muted, #aaa)';
        (b as HTMLElement).style.borderColor = isActive ? 'var(--accent-color, #4488ff)' : 'var(--border-color, #333)';
        (b as HTMLElement).style.fontWeight = isActive ? '700' : '500';
      });

      // Render the selected tab content
      const contentEl = document.getElementById('luckyTabContent');
      if (contentEl) {
        const renderer = luckyTabRenderers[tab];
        if (renderer) {
          try {
            contentEl.innerHTML = renderer();
          } catch (err) {
            console.error('[Lucky] Tab render error:', err);
            contentEl.innerHTML = '<div style="text-align:center;padding:24px;color:var(--text-muted,#888)">⚠️ Error loading tab</div>';
          }
        }
      }
    });
