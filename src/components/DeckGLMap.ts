...
export class DeckGLMap {
  ...
  constructor(container: HTMLElement, initialState: DeckMapState) {
    ...
    window.addEventListener('APP_FLY_TO_LOCATION', (e: Event) => {
      const { lat, lng, zoom } = (e as CustomEvent).detail;
      if (lat !== undefined && lng !== undefined) {
        this.setCenter(lat, lng, zoom || 8);
      }
    });
  ...
  private buildLayers(): LayersList {
    ...
    // Phase 4: Travel Destinations
    if (mapLayers.travelDestinations && TRAVEL_DESTINATIONS.length > 0) {
      layers.push(...this.createTravelDestinationsLayers());
    }
  ...
  private createTravelDestinationsLayers(): Layer[] {
    const layers: Layer[] = [];
    const isLight = getCurrentTheme() === 'light';

    // Only render contextual affiliate pins if we zoom into at least country/city level (zoom >= 5)
    // This maintains a clean global view and triggers the "See-and-Do" effect when flying into a location.
    if (!this.maplibreMap || this.maplibreMap.getZoom() < 5) return layers;

    // Background dot
    layers.push(new ScatterplotLayer({
      id: 'travel-destinations-dot',
      data: TRAVEL_DESTINATIONS,
      getPosition: (d) => [d.lon, d.lat],
      getFillColor: [255, 255, 255, isLight ? 230 : 200],
      getRadius: 16,
      radiusUnits: 'pixels',
      stroked: true,
      ...
      getLineWidth: 2,
      pickable: true,
      onClick: (info) => this.handleTravelDestinationClick(info.object),
    }));

    // Emoji text
    layers.push(new TextLayer({
      id: 'travel-destinations-text',
      data: TRAVEL_DESTINATIONS,
      getPosition: (d) => [d.lon, d.lat],
      getText: (d) => d.emoji,
      getSize: 18,
  ...
  private handleTravelDestinationClick(destination: any) {
    if (!destination) return;

    // Get cursor position for popup if possible, else center
    const center = this.getContainerCenter();
    this.popup.show({
      type: 'travelDestination',
      data: { ...destination, sourceVariant: SITE_VARIANT },
      x: center.x,
      y: center.y
    });
  }
