...
// Finance variant: Finance channels
const FINANCE_LIVE_CHANNELS: LiveChannel[] = [
  { id: 'bloomberg', name: 'Bloomberg', handle: '@markets', fallbackVideoId: 'iEpJwprxDdk' },
  { id: 'cnbc', name: 'CNBC', handle: '@CNBC', fallbackVideoId: '9NyxcX3rhQs' },
  { id: 'yahoo', name: 'Yahoo Finance', handle: '@YahooFinance', fallbackVideoId: 'KQp-e_XQnDE' },
  { id: 'fox-business', name: 'Fox Business', handle: '@FoxBusiness', fallbackVideoId: '1nO5D405hGk' },
];
...
export function getDefaultLiveChannels(): LiveChannel[] {
  if (SITE_VARIANT === 'tech') return TECH_LIVE_CHANNELS;
  if (SITE_VARIANT === 'finance') return FINANCE_LIVE_CHANNELS;

  if (!['world', 'finance', 'tech'].includes(SITE_VARIANT)) {
    if (SITE_VARIANT === 'travel' || SITE_VARIANT === 'sports') {
      return OPTIONAL_LIVE_CHANNELS.filter(c => ['bloomberg', 'cnn', 'sky', 'france24'].includes(c.id));
    }
    if (SITE_VARIANT === 'climate' || SITE_VARIANT === 'weather' || SITE_VARIANT === 'health') {
      return OPTIONAL_LIVE_CHANNELS.filter(c => ['bloomberg', 'sky', 'dw'].includes(c.id));
    }
    return OPTIONAL_LIVE_CHANNELS.slice(0, 4); // generic fallback
  }
  return FULL_LIVE_CHANNELS;
}

const DEFAULT_LIVE_CHANNELS: LiveChannel[] = getDefaultLiveChannels();
...
export class LiveNewsPanel extends Panel {
  ...
  private readonly ECO_IDLE_PAUSE_MS = 5 * 60 * 1000;
